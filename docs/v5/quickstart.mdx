...
<Note>
Most LLM providers (Google Gemini, OpenAI, Forge, Ollama, OpenRouter) are included by default. For additional providers, install extras: `uv tool install 'droidrun[anthropic,deepseek]'`.
</Note>
...
# For Forge (OpenAI-compatible model format: Provider/model-name)
...
**Common CLI flags:**
- `--provider` - LLM provider (GoogleGenAI, OpenAI, Forge, Anthropic, etc.)
